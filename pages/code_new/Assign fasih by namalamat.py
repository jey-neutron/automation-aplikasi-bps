...
def desc():
    ...
    return """1) Program untuk auto assign by selection di Fasih berdasarkan CSV dataframe. Nanti klo gada masalah akan lanjut ke sampel selanjutnya, klo ada sesuatu akan ada notenya (sementara ini kalo sampelnya dah ke-assign akan skip )
2) SSO untuk login Fasih
3) Nama survei isiin survei yang mau diassign, samain dengan nama di Fasih ya
4) Gsheet masukkan sheetID dan sheetname, tapi harus diedit tiap saat yah sebelum run program <br> Kolom df wajib: `'nm_sampel'`, `'alamat'`, `'email_petugas'`, `'usersso_pengawas'` <br> Liat filenya di link ini: <a href="https://docs.google.com/spreadsheets/d/1O0OUJkeGBJMTVeQ1MRLkYzGWCRqWGHyYBtDv4l-DK9I">Google Sheet</a>
            ...
            try:
                for dfrow in range(start,end): ## loop row df
                    ...
                    for i in range(1,jml_row): ## loop row fasih, cek row yang diinginkan
                        ...
                        #logger.info(f'{df.index[dfrow]}|↪️ idweb : '+id_web)
                        #logger.info(f'{df.index[dfrow]}|↪️ table : '+df.nm_sampel[dfrow]+df.alamat[dfrow])
                        #logger.info((df.nm_sampel[dfrow]+df.alamat[dfrow] == id_web))
                    ...
                    if (tick == 1): #then untick
                        #driver.find_elements_by_css_selector('.ng-star-inserted:nth-child('+str(i)+') > td > .ng-untouched')[0].click()
                        #metode tick all then untick all
