...
def desc():
    ...
    return """1) Program untuk mendapatkan data dari isian fasih SKTNP.   
...
def RUN(ssoname, ssopass, tab_survei, pilihan_survei, df_name,sheet_name, rentang, hide_ff, close_ff=True):
    try:
        ...
        #jml_row = 10+1
        if "-" in str(rentang):
            i0 = int ( str(rentang).split('-')[0] )
            i0 = 1 if i0<1 else i0
            jml_row = int ( str(rentang).split('-')[1] )+1
        else :
            i0 = 1
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        df = df._append(pd.DataFrame([[now]+[i]+blok1+["",nm_usaha, "Data ga lengkap: "+str(e)]+[None]*51 ], columns=columns), ignore_index=True)
                        break
                    ...
                    df = df._append(pd.DataFrame([[now]+[i]+blok1+blok21+blok22+blok3[0]+blok3[1]+blok3[2]+blok3[3]+blok4+[blok5] ], columns=columns), ignore_index=True)
                ...
                ## jika gagal print error
                    ...
                    df = df._append(pd.DataFrame([[now]+[i]+["","",nm_usaha, "GAGAL: "+str(e)]+[None]*59  ], columns=columns), ignore_index=True)
                    driver.close()
                    driver.switch_to.window(driver.window_handles[0])
                    gagal += 1
                    if gagal >= 50:
